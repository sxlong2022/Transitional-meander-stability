...
def make_openchannel_profile(
    ...
    - mode="laminar_curved"： solve_os 
      U(y) = (1.0 + gamma) * (-0.25*y**2 + 0.5*y + 0.75)， gamma Parameters。
    ...
    if mode == "empirical_power":
        # Parameters U(z) = z^p * ((1+p) - p z), z \in [0,1]
        # Satisfying U(0)=0, U(1)=1, U'(1)=0。Parameters p Re_scalar ：
        # Re “”，Re 。
    ...
    if mode == "zs_turbulent":
        ...
        if Cf_eff is None or Cf_eff <= 0.0:
            raise ValueError("make_openchannel_profile(mode='zs_turbulent') requires Cf_scalar and Re_scalar")
    ...
    if mode == "mcmm_profile":
        # MCMM U(y)， Chebyshev grid
        # params mcmm_profile_path mcmm_profile_data
        # ： laminar_ref
        #
        # ：
        # 1. load_os_profile_from_mcmm read (y_mcmm, U_mcmm)
        # 2. scipy.interpolate Chebyshev grid y
        # 3. D, D2 calculate U_y, U_yy
        #
        # ：Directly use laminar_ref
        U = -0.25 * y ** 2 + 0.5 * y + 0.75
        U_y = D @ U
        U_yy = D2 @ U
        return U, U_y, U_yy

        raise ValueError(f"make_openchannel_profile: Not implemented, mode={mode!r}")
...
def solve_os(
    ...
    if bc_type not in {"rigid", "open_rigid_lid", "open_free_surface"}:
        raise ValueError(f"Unknown bc_type={bc_type!r}, must be 'rigid', 'open_rigid_lid', or 'open_free_surface'")
    ...
    try:
        ...
        raise ImportError("solve_os requires SciPy (scipy.linalg), please install it first") from exc
    ...
    if bc_type == "open_free_surface":
        if Fr is None:
            raise ValueError("bc_type='open_free_surface' requires Fr in params")
        ...
        if callable(delta_Ac_assembler):
            ...
            if delta_A is not None:
                assert delta_A.shape == A_ext.shape, f"_delta_Ac_assembler returned matrix shape {delta_A.shape} does not match A_ext shape {A_ext.shape}"
                A_ext = A_ext + delta_A
