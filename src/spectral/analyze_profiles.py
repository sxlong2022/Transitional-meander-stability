"""Spectral and spatial correlation analysis of channel width B(s) and curvature C(s) profiles.

Processes continuous 26-year primary channel trunk profiles (2000–2025) across the Gaocun–Sunkou reach:
  * Single-sided Fast Fourier Transform (FFT) and Power Spectral Density (PSD)
  * Identification of dominant planform wavelengths (lambda_B and lambda_C)
  * Cross-correlation and spatial phase lag between B(s) and C(s)
  * Spatial persistence scales (e-folding autocorrelation length)

Outputs:
  - results/spectral/spectral_summary.csv (multi-decadal summary)
  - results/spectral/{year}_B_spectrum.csv (annual width spectrum)
  - results/spectral/{year}_C_spectrum.csv (annual curvature spectrum)

Usage:
    python -m src.spectral.analyze_profiles
...
def _save_spectrum_csv(spec: Dict[str, np.ndarray], out_path: Path,
                       label: str) -> None:
    """Save single signal FFT spectrum to CSV."""
    if spec["freq"].size == 0:
        return
    df = pd.DataFrame({
        "freq_1m": spec["freq"],
...
def analyze_all_trunks(
    ...
    """Batch analyze channel trunk profiles across all survey years.

    Returns DataFrame with one row per year, containing all SpectralResult fields.
    """
    trunk_dir = trunk_dir or TRUNK_DIR
    out_dir = out_dir or SPECTRAL_DIR
    out_dir.mkdir(parents=True, exist_ok=True)

    # Discover trunk files
